...
class PasienPerKecamatanController extends BaseController
{    
    ...
    public function actionExport()
    {
        ...
        $sheet->setCellValue('A2', 'Laporan Kunjungan Pasien Per Kecamatan');
        ...
        $fileName = 'kunj-pasien-per-kecamatan.xlsx';
    ...
    {
        $query .= " WHERE DATE(tglclosingkasir) between :datefrom and :dateto AND ";
        
        if ($this->instalasi == '9999') {
            $ids = array_keys($this->listinstalasi());
            $key = array_search('9999', $ids);
            unset($ids[$key]);
            $glueIds = implode(',', $ids);

            $query .= 'im.instalasi_id in ('. $glueIds . ')';
        } else {
            $query .= ' im.instalasi_id = :instalasi ';
        }
